...
| Harish | 1992-06-04, 03:10 AM IST, Pallipattu (Tiruvallur), Tamil Nadu — 13.33°N, 79.45°E | 辛 신 (Yin Metal) | Water (水) *(engine heuristic, 조후 climate-check applied)* | [harish/harish-report.md](reports/harish/harish-report.md) + [career.md](reports/harish/career.md) — PDFs: [base](reports/harish/harish-report.pdf) · [combined](reports/harish/harish-combined.pdf) · [career](reports/harish/career.pdf). Deep Destiny (engine `--tier deep`, `--longitude 79.4408 --city "Pallipattu, Tamil Nadu"`, no override). **2026-09-19:** entire candidate folder deleted and rebuilt from scratch per user request. Pillars: 壬申/乙巳/辛亥/己丑, Balanced DM, 용신 Water/희신 Metal, 정관격. `career.md` went through two passes the same day: an initial short version (user feedback: "very short... needs deep details... specify domains for business and jobs, suitable roles, investment patterns, best fields and career transition timing") was replaced with a full standalone deep-dive covering ranked career domains, concrete job roles, business-domain fit, investment/wealth behavior, and a decade-by-decade + annual career-transition timing map (target window: ages 36-39 near-term, ages 50-69 for a major independent launch). Also fixed a `daeun_overlay.py` bug found while building the timing section (favorable-lean labels used the raw pre-climate candidate, not the resolved 용신) — see the engine-level entry below; base report/combined/PDFs rebuilt again after that fix. No `luck-timeline.md` or `land-workshop-business.md` this pass. |
