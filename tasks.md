...
**Test count:** 1139 pytest cases passing / 9 xfailed (`python3 -m pytest`); engine validation gate
`python3 tools/run_validation.py` → 205 checks (201 PASS / 4 INTERPRETATION / 0 FAIL); `ruff check
...
**2026-09-26 audit (N-1..N-22):** 21 of 22 fixed (N-21 is not in this repo). N-6 (조후 gated on
the chart: option A) and N-12 (월률분야 branch weights) were implemented by user decision. **Client
reports need regenerating** (Element Balance %s and two compat scores moved). See
