...
@pytest.mark.parametrize(
    ...
    [
        # N-2 (2026-09-26 audit): 2024 立春 is 17:27:08 KST (packaged
        # ephemeris table, N-3; sajupy's CSV said 17:00). Seoul (longitude 127, utc_offset 9) has a real solar
        ...
        # civil-converted term instant, so civil births up to ~30 min after
        # the term were wrongly pushed into the previous year/month (癸卯/乙丑)
        # instead of 甲辰/丙寅.
        (17, 20, "癸卯", "乙丑"),  # before the true term (N-3; sajupy's 17:00 put it after)
        (17, 30, "甲辰", "丙寅"),  # after the term civilly; solar time (~16:58) isn't
        (17, 55, "甲辰", "丙寅"),
    ],
)
...
@pytest.mark.parametrize(
    "hour,minute,expected_year,expected_month",
    [
        # Same 2024 立春 (17:27:08 KST), converted to IST (utc_offset 5.5):
        # 13:57:08 IST. Mumbai's real longitude (72.9) sits ~38 min west of
        # the +5.5 standard meridian (82.5), so a civil birth at 13:58-14:30
        # IST (after the term) has a SOLAR time ~40 min earlier (before the
        # term), which the bug used to compare instead of the civil clock.
        (13, 50, "癸卯", "乙丑"),  # before the term either way
        (13, 58, "甲辰", "丙寅"),  # after the term civilly; solar time still isn't
        (14, 30, "甲辰", "丙寅"),
    ],
)
